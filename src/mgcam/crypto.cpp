...
bool cRSA::Input(BIGNUM *d, const unsigned char *in, int n, bool LE) const
{
  if(LE) {
    unsigned char tmp[n];
...
int cRSA::Output(unsigned char *out, int n, BIGNUM *r, bool LE) const
{
  int s=BN_num_bytes(r);
  if(s>n) {
    dc(printf("rsa: RSA len %d > %d, truncating\n",s,n))
    unsigned char buff[s];
    BN_bn2bin(r,buff);
    memcpy(out,buff+s-n,n);
    }
  else if(s<n) {
    dc(printf("rsa: RSA len %d < %d, padding\n",s,n))
    int l=n-s;
    memset(out,0,l);
    BN_bn2bin(r,out+l);
    }
  else BN_bn2bin(r,out);
  if(LE) RotateBytes(out,n);
  return s;
}
...
void cDes::Des(unsigned char *data, const unsigned char *key, int mode) const
{
  ...
  unsigned int C=bswap_32(get_unaligned((unsigned int *)(key  ))) >> 4;
  unsigned int D=bswap_32(get_unaligned((unsigned int *)(key+3))) & 0xfffffffL;
...
#ifdef DEBUG_CRYPTO
  char str[64];
  printf("aes: AES activated key=%s\n",HexStr(str,key,16));
#endif
}
...
{
  if(active) {
    if(len&15) dc(printf("aes: AES crypted message length not a multiple of 16\n"))
...
#include "support/rc6.c"

void cRC6::SetKey( unsigned char *key )
{
	rc6initkey( &rc6_key, key, 16 );
}

void cRC6::Decrypt( unsigned char *data )
{
	rc6decrypt( &rc6_key, data );
}
